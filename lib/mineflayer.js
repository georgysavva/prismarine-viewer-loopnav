...
module.exports = (bot, { viewDistance = 6, firstPerson = false, port = 3000, prefix = '' }) => {
  ...
  io.on('connection', (socket) => {
    ...
    bot.on('sentPosAction', function (lastsentpos) {
      const packet = { pos:lastsentpos, yaw: lastsentpos.yaw, addMesh: true}
      if (firstPerson) {
        packet.pitch = lastsentpos.pitch
      }
      socket.emit('position', packet)
      worldView.updatePosition(lastsentpos)
    })

    worldView.listenToBot(bot)
    socket.on('disconnect', () => {
      // bot.removeListener('move', botPosition)
      bot.removeAllListeners('sentpos')
